...
public class GeoBlockAnalyzer {
  ...
  private void populateCoordinatesMap() {
    for (final Map.Entry<Integer, Geo> entry : this.geoMap.entrySet()) {
      final int key = entry.getKey();
      final Geo geo = entry.getValue();
      final int x = key % this.width;
      final int y = key / this.width;
      geo.setCoordinates(x, y);
